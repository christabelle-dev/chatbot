...
if "viewing_session_index" not in st.session_state:
    st.session_state.viewing_session_index = None  # session currently being viewed
...
with st.sidebar:
    ...
    if st.button("➕ Start New Session"):
        ...
        st.session_state.viewing_session_index = None
        st.rerun()
    ...
    for i, session in enumerate(st.session_state.all_sessions):
        ...
        with cols[1]:
            if st.button("👁️ View", key=f"view_{i}"):
                st.session_state.viewing_session_index = i
                st.rerun()
        with cols[2]:
            if st.button("❌", key=f"del_session_{i}"):
                ...
                if i == st.session_state.current_session_index:
                    st.session_state.quotes = []
                if i == st.session_state.viewing_session_index:
                    st.session_state.viewing_session_index = None
...
def generate_quote(theme_text: str) -> str:
    ...
    prompt = (
        ...
        f"1–3 sentences max, positive, practical, original."
    )
...
# --- Display Viewing Session Quotes with Delete Buttons ---
if st.session_state.viewing_session_index is not None:
    st.subheader(f"📜 Quotes in Session {st.session_state.viewing_session_index + 1}")
    viewing_session = st.session_state.all_sessions[st.session_state.viewing_session_index]
    if not viewing_session:
        st.info("No quotes in this session yet.")
    else:
        for i, q in enumerate(viewing_session):
            quote_cols = st.columns([6, 1])
            with quote_cols[0]:
                st.markdown(f"**Theme:** {q['theme']}")
                st.write(q["content"])
            with quote_cols[1]:
                if st.button("❌ Delete", key=f"view_del_{i}"):
                    del viewing_session[i]
                    st.session_state.all_sessions[st.session_state.viewing_session_index] = viewing_session
                    st.rerun()
            st.divider()
